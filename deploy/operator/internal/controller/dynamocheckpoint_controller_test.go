...
func volumeMountMap(mounts []corev1.VolumeMount) map[string]string {
	out := make(map[string]string, len(mounts))
	for _, mount := range mounts {
		out[mount.Name] = mount.MountPath
	}
	return out
}
...
func TestBuildCheckpointJob(t *testing.T) {
	...
	assert.Equal(t, "manual-checkpoint", mainEnv[consts.DynamoNamespaceEnvVar])
	assert.Equal(t, consts.ComponentTypeWorker, mainEnv[consts.DynamoComponentEnvVar])
	assert.Equal(t, "worker-1234", mainEnv[consts.DynamoNamespaceWorkerSuffixEnvVar])
	assert.Equal(t, "kubernetes", mainEnv[consts.DynamoDiscoveryBackendEnvVar])
	assert.Equal(t, "9090", mainEnv["DYN_SYSTEM_PORT"])
	assert.Equal(t, "true", mainEnv["DYN_SYSTEM_ENABLED"])
	...
	mountPaths := volumeMountMap(main.VolumeMounts)
...
func TestBuildCheckpointJobUsesTargetContainerName(t *testing.T) {
	...
	assert.NotContains(t, envMap(main.Env), snapshotprotocol.SnapshotControlDirEnv)
	assert.NotContains(t, volumeMountMap(main.VolumeMounts), snapshotprotocol.SnapshotControlVolumeName)

	require.NotNil(t, target.ReadinessProbe)
	assert.Equal(t, snapshotprotocol.SnapshotControlMountPath, envMap(target.Env)[snapshotprotocol.SnapshotControlDirEnv])
	assert.Equal(t, "1", envMap(target.Env)["USER_ENV"])
	assert.Equal(t, snapshotprotocol.SnapshotControlMountPath, volumeMountMap(target.VolumeMounts)[snapshotprotocol.SnapshotControlVolumeName])
	assert.Equal(t, consts.PodInfoMountPath, volumeMountMap(target.VolumeMounts)[consts.PodInfoVolumeName])
}
...
func TestBuildCheckpointJobAddsGMSCheckpointClient(t *testing.T) {
	...
	mainMounts := volumeMountMap(main.VolumeMounts)
	...
	saverMounts := volumeMountMap(saver.VolumeMounts)
	assert.Equal(t, gms.SharedMountPath, saverMounts[gms.SharedVolumeName])
}
...
func TestBuildCheckpointJobInjectsStandardEnvVars(t *testing.T) {
	...
	assert.Equal(t, "nats://custom:4222", mainEnv["NATS_SERVER"])
	assert.Equal(t, "10090", mainEnv["DYN_SYSTEM_PORT"])
	assert.Equal(t, "http://etcd:2379", mainEnv["ETCD_ENDPOINTS"])
	assert.Equal(t, "http://model-express:8000", mainEnv["MODEL_EXPRESS_URL"])
	assert.Equal(t, "http://prometheus:9090", mainEnv["PROMETHEUS_ENDPOINT"])
}
