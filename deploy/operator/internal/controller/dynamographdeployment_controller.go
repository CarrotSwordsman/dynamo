...
func (r *DynamoGraphDeploymentReconciler) reconcileCheckpoints(
	...
	for i := range dynamoDeployment.Spec.Components {
		...
		if err := overlayServiceGMSRestoreClients(info, dynamo.GetGPUMemoryService(component)); err != nil {
			return nil, nil, fmt.Errorf("failed to apply checkpoint gpuMemoryService restore config for component %s: %w", componentName, err)
		}
...
func (r *DynamoGraphDeploymentReconciler) createCheckpointCR(
	...
	if converted := dynamo.ToAlphaGPUMemoryService(dynamo.GetGPUMemoryService(component)); converted != nil {
