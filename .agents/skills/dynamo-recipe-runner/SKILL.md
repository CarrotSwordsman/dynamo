...
description: Run existing NVIDIA Dynamo Kubernetes recipes end to end. Use when a user wants to select, patch, deploy, smoke test, or get started with a Dynamo recipe for a model, backend, GPU type/count, deployment mode, or router mode.
---
...
```bash
git status --short
python3 .agents/skills/dynamo-recipe-runner/scripts/recipe_tool.py list --format table
...
```bash
python3 .agents/skills/dynamo-recipe-runner/scripts/recipe_tool.py list \
...
```bash
python3 .agents/skills/dynamo-recipe-runner/scripts/recipe_tool.py validate \
...
If this fails, switch to `dynamo-troubleshoot`.
